...
28. **Draft/publish workflow in the editor** — `slides.status` (`draft`|`published`) is the unit of publishing; students only see `published` (RLS). New content (new slides, duplicates) is created as **draft**. Editing published content (a slide's own fields, or any block on it) reverts THAT slide to draft until the admin clicks Publish — implemented centrally in `handleSave` via `contentChangedSlideIds()` + the store's `markSlidesDraft()` (keyed off a content-only fingerprint that excludes `status`/`order_index`, so publishing/reordering never counts as an edit). Publish (`publishCourse`) flips the whole course's live slides to published. Draft badges + the content-health chip in the editor make unpublished content impossible to miss (so draft-by-default is safe, not a "phantom draft").

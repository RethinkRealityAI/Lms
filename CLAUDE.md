...
src/
  ...
  lib/
    ...
    profile/
      professions.ts        # "I am a" options + resolve/split helpers (signup + profile share it)
    referral/
...
| 069 | signup_profession | **Profession collected at signup, and it is now REQUIRED.** The signup form drops the Learner/Instructor role picker entirely (everyone who signs up is a student; instructors/admins are provisioned by invite or role upgrade, so the instructor verification-code path is gone too) and asks **"I am a"** instead: Physician / Nurse / Allied health professional / **Other** + free text. Shared definition in `lib/profile/professions.ts` (`resolveProfession` collapses the select + free-text into one stored value and REJECTS an unknown or half-filled answer; `splitStoredProfession` is the inverse, so the profile form round-trips legacy free-text values like `RN` back into Other without losing them). Stored in the EXISTING `users.occupation` column — the UI says "profession", the column keeps its name so no data, export, admin tool or query moves. `handle_new_user` v5 reads `profession` from signup metadata, trims and caps it at 80 chars (last gate before it reaches a public report) and tolerates its absence. **`claim_legacy_profile` flipped to `occupation = coalesce(occupation, v_legacy.occupation)`** — it previously let the EdApp import WIN, so a returning learner who picked "Physician" would have it silently replaced by their old free-text value, fragmenting the very breakdown this migration exists to populate; now the fresh answer wins and the import is only a fallback (matches how `country` already behaved in the same statement). WHY REQUIRED: the referral report's "who is signing up" breakdown read an optional profile field only ~3% of learners had ever filled in — real clinician signups from SickKids/CHEO/HHS all had `occupation` NULL. The student profile page uses the same dropdown so a later edit cannot reintroduce ungroupable values |
| 071 | referral_source_tracking | **Traffic sources + honest open counts + outreach denominator.** `referral_visits` gains `source_category`/`referrer_host` (HOST only, never path/query)/`campaign` (`?s=` tag); `users` gains the coarse `referral_source_category`/`referral_campaign` (never a host — identified rows stay coarse), protected by `guard_referral_attribution` v2. `record_referral_visit` v2 (7 params; old 4-arg overload DROPPED) normalises everything in SQL — it is anon-callable, so app-side normalisation (`lib/referral/sources.ts`) cannot be the only gate. `handle_new_user` v6 stamps channel+campaign only when the referral code resolves. `get_referral_report` v4 adds `sources`/`referrers`/`referrers_withheld`/`campaigns`; named referring sites sit behind a k-anonymity floor (≥3 distinct browsers) with the withheld remainder reported as counts. **Counting fix:** "link opens" summed daily uniques = browser-days (3-day repeat visitor = 3 opens; error grew with window length, deflating the funder-facing conversion rate) — `totals.visits`, `lifetime.visits`, `admin_referral_overview.visits` and all 071 breakdowns now `count(DISTINCT visitor_key)`; the daily chart keeps per-day counts. `referral_codes.outreach_reached`/`outreach_note`: ambassador-REPORTED audience size ("presented to 40 people") — the denominator no log can recover, always labelled self-reported on the report. Cross-browser fingerprint dedupe (IP+UA hash) was evaluated and REJECTED — and the rejection survives the audience being on PERSONAL devices (phones, home wifi), not hospital fleets: mobile carriers put thousands of unrelated subscribers behind one CGNAT IP with near-identical frozen mobile UAs (two iPhone users on the same carrier hash the same → merged), while the dupe people actually produce on personal devices — phone + laptop — has different UA AND different network, so no fingerprint can merge it anyway. Distinct-browser cookie counting is already near-person-accurate there (personal browsers keep the 400-day cookie). Do not re-introduce fingerprinting under either audience assumption. Report copy states the browser unit instead |
| 072 | referral_period_activity | **Period-framed activity + calendar-month view.** `get_referral_report` v5 adds `period_activity` — active learners, lessons completed, modules completed (final lesson lands in window), certificates issued (`issued_at` in window) — counted over EVERYONE ever attributed, windowed by when the activity happened; the complement of the cohort funnel (a March signup finishing modules in August now shows in August). Report gains a month picker (`?range=YYYY-MM`, helpers in `lib/referral/constants.ts`) + "What happened in <month>" card → the monthly report is just the dashboard with a month selected. Report page gains the tag cheat sheet (`CAMPAIGN_CHEAT_SHEET` in `lib/referral/sources.ts` — every tag must survive `normalizeCampaign` unchanged, tested). The 071 ambassador-reported outreach denominator (`outreach_reached`/`outreach_note`) is RETIRED from the UI (too much upkeep to ask of ambassadors) — columns and RPC fields remain, nothing renders or edits them. recharts Areas get `isAnimationActive={false}`: a resize or immediate print-to-PDF caught the series mid-entrance-animation and exported an EMPTY chart |
...
- **`required` (default true)** — "Required to continue" toggle in the quiz editor. Required quizzes block the Next button **on their own slide** (amber hint: "Answer the quiz on this slide correctly to continue") and lesson completion; `required: false` = practice quiz, never gates. The gate only ever counts quizzes on slides the student can actually see (`currentSlides`) — quiz blocks stranded on soft-deleted or draft slides can NOT brick a lesson (they survive slide deletion in `lesson_blocks`; this bricked real users pre-fix). Already-completed lessons never re-gate, and correct answers rehydrate from `quiz_block_responses` on reload. **A passing answer is STICKY**: `persistAnswer` (`quiz-inline/viewer.tsx`) never downgrades `is_correct` true→false on a later wrong re-attempt (all four gating types compute one exact-match correctness for both the gate and persistence), and it retries a dropped write — because `issue_course_certificate` trusts the DB `is_correct`, so the in-memory gate and the cert must never diverge (they once did: a wrong re-attempt overwrote a pass → "100% complete, no certificate"). Belt-and-suspenders, the final completion slide **self-heals** — it re-attempts the idempotent cert RPC, so a learner who fixes a stale answer gets their certificate (and celebration) on revisit; the survey page surfaces the RPC error instead of a silent "thank you".
...
- `bg-card` is **transparent** in this Tailwind v4 setup (no `--color-card` token) — quiz cards use explicit `bg-white` so they stay readable on dark glass. **The same trap applies to `bg-popover` and `bg-accent`**: `--popover`/`--accent` exist in `globals.css` as bare HSL triples but `--color-popover`/`--color-accent` are never mapped, so every `<Select>` dropdown in the app rendered TRANSPARENT (options unreadable over the content behind them) with no hover or selected-row feedback. `components/ui/select.tsx` now sets explicit `bg-white` / `border-slate-200` / `hover:bg-slate-100`. Prefer explicit colours over shadcn tokens here unless you have checked the token is mapped.
...
`/r/<code>` classifies the `Referer` header (`lib/referral/sources.ts` — email /
social / messaging / search / website / direct; own-site → direct) and records
host+channel+`?s=` campaign tag on the visit; the channel rides `lms_ref_src`
(same last-touch window as `lms_ref`) onto the signup. Desktop email apps, QR
scans and most messengers send NO referrer — that is why campaign tags exist:
the admin card offers copyable tagged links and the QR download is tagged
`?s=qr`. The report's "How people arrived" section shows channels
(opens+accounts), named referring sites (k-anonymity floored), and tags.
"Link opens" counts DISTINCT browsers for the period (approximate, says so on
the page); every stage below it counts real accounts and is exact.

...
import { withInstitutionSlug } from '@/lib/tenant/path';
...
function GansidHero({ slug }: { slug: string }) {
  return (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <Button size="lg" asChild className="w-full sm:w-auto h-16 rounded-2xl px-10 text-lg font-bold shadow-2xl shadow-red-200 bg-[#DC2626] hover:bg-[#991B1B] active:scale-95 transition-all">
                <Link href={withInstitutionSlug('/login?tab=signup', slug)}>Get Started <ArrowRight className="ml-2 h-5 w-5" /></Link>
              </Button>
...
function GansidPrograms({ slug }: { slug: string }) {
  return (
    <section id="programs" className="py-20 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <Link href={withInstitutionSlug('/patient-organizations', slug)} className="group">
          ...
          {/* Clinicians Program */}
          <Link href={withInstitutionSlug('/clinicians', slug)} className="group">
...
function ScagoHero({ slug, branding }: { slug: string; branding: ReturnType<typeof getInstitutionBranding> }) {
  return (
        ...
        <div className="grid lg:grid-cols-[1.05fr_1fr] gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-8">
              ...
              >
                <Link href={withInstitutionSlug('/login?tab=signup', slug)}>
            ...
            <p className="text-sm text-slate-500">
              ...
              <Link
                href={withInstitutionSlug('/login', slug)}
...
function ScagoHowItWorks({ slug }: { slug: string }) {
  const steps = [
    {
      icon: UserPlus,
      step: '01',
...
function ScagoCurriculum({ slug }: { slug: string }) {
  return (
...
function ScagoAccreditation({ slug }: { slug: string }) {
  return (
        ...
        <div className="relative overflow-hidden rounded-[2rem] bg-[#1A1A1A] px-8 py-12 md:px-14 md:py-14">
          ...
          <div className="relative z-10 grid lg:grid-cols-[1.4fr_1fr] gap-10 items-center">
            ...
            <div className="flex flex-col sm:flex-row lg:flex-col gap-3 lg:items-stretch">
              ...
              >
                <Link href={withInstitutionSlug('/login', slug)}>Sign in</Link>
              </Button>
            </div>
          </div>
        </div>
...
export function LandingPage({ institutionSlug }: { institutionSlug: string }) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col selection:bg-red-100 selection:text-red-900">
      <PublicNav institutionSlug={institutionSlug} />

      {isScago ? (
        <>
          <ScagoHero slug={slug} branding={branding} />
          <ScagoStats />
          <LandingNotification institutionSlug={institutionSlug} />
          <ReturningLearnerSection
            institutionSlug={institutionSlug}
            signInHref={withInstitutionSlug('/login', slug)}
            signUpHref={withInstitutionSlug('/login?tab=signup', slug)}
          />
          <ScagoOverview />
          <ScagoHowItWorks slug={slug} />
          <ScagoCurriculum slug={slug} />
          <ScagoAccreditation slug={slug} />
        ...
        <>
          <GansidHero slug={slug} />
          <LandingNotification institutionSlug={institutionSlug} />
          <GansidPrograms slug={slug} />

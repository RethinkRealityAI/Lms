...
- **Quiz answer persistence (`is_correct`) ↔ cert gate consistency** — `issue_course_certificate` trusts `quiz_block_responses.is_correct`, so it must never diverge from the in-memory completion gate. A passing answer is **sticky** (`persistAnswer` never downgrades `is_correct` true→false on a later wrong re-attempt), the write is **retried** (a dropped write can't leave the gate satisfied but the DB unwritten), and the final completion slide **self-heals** issuance (idempotent RPC re-attempt). Divergence once stranded a learner at 100% complete with no certificate (dapo, SCAGO Module 1). All four gating quiz types compute one exact-match correctness for both the gate and persistence.
...
| Certified learner blocked from their certificate | A wrong quiz re-attempt overwrote `is_correct` true→false while the in-memory gate + lesson completion stayed satisfied; the cert RPC (trusts DB `is_correct`) then refused, and the survey page swallowed the error → "100% complete, no cert, no explanation" | Sticky-correct persistence (no true→false downgrade) + write retry + completion-slide self-heal issuance + survey-page error surfacing |
